...
import { updateProfile, uploadAvatar, upsertSportPreference, getSuggestedSports } from "@/actions/profile"
...
export function ProfileEditForm({ profile }: { profile: ProfileWithPrefs }) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { name: profile.name, bio: profile.bio ?? "", location: profile.location ?? "" },
  })
  ...
  async function handleAddSport() {
    ...
    const result = await upsertSportPreference(selectedSport as Sport, selectedSkill)
    if (result?.success) {
      toast.success(`${SPORT_LABELS[selectedSport as Sport]} added!`)
      setPreferences((prev) => {
        const filtered = prev.filter((p) => p.sport !== selectedSport)
        return [...filtered, { sport: selectedSport as Sport, skillLevel: selectedSkill, id: "", userId: profile.id, createdAt: new Date(), updatedAt: new Date() }]
      })
      setSelectedSport("")
    }
  }
  ...
  async function handleAddSuggested(sport: Sport) {
    ...
    setPreferences((prev) => {
      ...
      return [...filtered, { sport, skillLevel: SkillLevel.BEGINNER, id: "", userId: profile.id, createdAt: new Date(), updatedAt: new Date() }]
    })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-border bg-card">
        ...
        <CardContent className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">JPG, PNG or WebP. Max 4MB.</p>
          </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField control={form.control} name="bio" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Tell people about yourself and what sports you love..." rows={3} {...field} />
                  </FormControl>
                  <FormMessage />
                  <Button type="button" variant="outline" size="sm" onClick={handleSuggestSports} disabled={loadingSuggestions} className="mt-1">
                    {loadingSuggestions ? <Loader2 className="h-3 w-3 animate-spin mr-1" /> : <Sparkles className="h-3 w-3 mr-1 text-primary" />}
              ...
              {suggestedSports.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  <span className="text-xs text-muted-foreground w-full">Detected sports — click to add:</span>
                  {suggestedSports.map((s) => (
                    <Badge key={s} variant="outline" className="cursor-pointer hover:bg-primary hover:text-primary-foreground" onClick={() => handleAddSuggested(s)}>
                      + {SPORT_LABELS[s]}
                    </Badge>
                  ))}
                </div>
      ...
      <Card id="sports" className="border-border bg-card">
        <CardHeader><CardTitle className="text-base">Add a Sport</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-3 flex-wrap">
            ...
            <Select value={selectedSkill} onValueChange={(v) => setSelectedSkill(v as SkillLevel)}>
              <SelectTrigger className="w-40">
            ...
            <Button onClick={handleAddSport} disabled={!selectedSport}>Add</Button>
          </div>
